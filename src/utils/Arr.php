<?php namespace nyx\utils;
	 ...
	 * the default delimiter set statically in this class will be used.
	 *
	 * Some code in this class can be simplified and some duplication could be avoided but it is laid our so that the
	 ...
	 *
	 * @package     Nyx\Utils\Arrays
	 * @version     0.0.8
	 * @author      Michal Chojnacki <m.chojnacki@muyo.pl>
	 * @copyright   2012-2014 Nyx Dev Team
	 * @link        http://docs.muyo.pl/nyx/utils/arrays.html
	...
	class Arr
	{
		...
		public static function all(array $array, callable $callback, $strict = true)
		{
		...
		public static function any(array $array, callable $callback, $strict = true)
		{
		...
		public static function average(array $array, $decimals = 0)
		{
		...
		public static function contains(array $haystack, $needle, $strict = true)
		{
			foreach($haystack as $value)
			{
				if((!$strict and $needle == $value) or $needle === $value) return true;

				if(is_array($value) and static::contains($needle, $value, $strict)) return true;
			}
		...
		public static function delimit(array $array, $prepend = '', $delimiter = null)
		{
			// Results holder.
			$results = [];

			// Which string delimiter should we use?
			if(null === $delimiter) $delimiter = static::$delimiter;
		...
		public static function every(array $array, callable $callback, $strict = true)
		{
		...
		public static function fetch(array $array, $key, $delimiter = null)
		{
			// Results holder.
			$results = [];
		...
		public static function first(array $array, $callback = false, $default = null)
		{
			...
			if(!$callback) return reset($array);

			// With a callable given, return the first value which passes the given truth test.
			if(is_callable($callback)) return static::find($array, $callback, $default);
		...
		public static function get(array $array, $key = null, $default = null, $delimiter = null)
		{
			...
			foreach(explode($delimiter, $key) as $segment)
			{
				if(!array_key_exists($segment, $array)) return $default;
		...
		public static function has(array $array, $key, $delimiter = null)
		{
			// Which string delimiter should we use?
			if(null === $delimiter) $delimiter = static::$delimiter;
			...
			foreach(explode($delimiter, $key) as $segment)
			{
				if(!is_array($array) or !array_key_exists($segment, $array)) return false;
		...
		public static function initial(array $array, $callback, $default = null)
		{
			...
			if(is_callable($callback))
			{
				...
				{
					if(!call_user_func($callback, $key, $value)) break;
		...
		public static function merge(array $array, array $with)
		{
			...
			// We don't need the initial array in that set.
			unset($arrays[0]);
		...
		/**
		 * Returns a string delimited key from an array and then removes it.
		 *
		 * @param   array   $array      The array to search in.
		 * @param   string  $key        The string delimited key.
		 * @param   string  $delimiter  The delimiter to use when exploding the key into parts.
		...
		public static function pull(&$array, $key, $delimiter = null)
		{
		...
		public static function random(array $array, $number = null)
		{
			...
			if(!$number) return $array[array_rand($array)];
		 ...
		 *
		 * @param   array   $array      The array to search in.
		 * @param   mixed   $key        The string delimited key.
		 * @param   string  $delimiter  The delimiter to use when exploding the key into parts.
		 */

		public static function remove(array &$array, $key, $delimiter = null)
		{
			// Which string delimiter should we use?
			if(null === $delimiter) $delimiter = static::$delimiter;
		...
		public static function rest(array $array, $callback = false, $default = null)
		{
			...
			if(2 > count($array)) return $default;

			// For a falsy callback, return all but the first element of the array.
			if(!$callback) return array_slice($array, 1);
		...
		public static function set(array &$array, $key, $value, $delimiter = null)
		{
			...
			while($key = array_shift($keys))
			{
				if(!isset($array[$key]) or !is_array($array[$key])) $array[$key] = [];
		...
		public static function some(array $array, callable $callback, $strict = true)
		{
		 ...
		 *
		 * @param   array   $array      The array to traverse.
		 * @param   mixed   $values     The values which should get removed. Treat each value as a separate argument
		 *                              for this method.
		 * @return  array
		 */

		public static function without(array $array, $values)
		{
			$values = func_get_args();

			// We don't need the initial array within the values, obviously.
			unset($values[0]);
