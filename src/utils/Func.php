<?php namespace nyx\utils;
	 ...
	 * Func
	 *
	 * Utilities related to functions/methods/callables.
	 *
	 * @package     Nyx\Utils\Func
	 * @version     0.0.4
	 * @author      Michal Chojnacki <m.chojnacki@muyo.pl>
	 * @copyright   2012-2014 Nyx Dev Team
	 * @link        http://docs.muyo.pl/nyx/utils/func.html
	...
	class Func
	{
		...
		/**
		 * @var array   The results cache for self::memoize().
		 */
		...
		public static function after($times = 1, callable $callback)
		{
			return function() use ($callback, $times)
			{
				...
				if(++$count >= $times) return call_user_func_array($callback, func_get_args());
			};
		}
		...
		public static function only($times = 1, callable $callback)
		{
			return function() use ($callback, $times)
			{
				...
				if($times >= ++$called) return call_user_func_array($callback, func_get_args());
			};
		}
