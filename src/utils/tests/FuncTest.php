<?php namespace nyx\utils\tests;
	...
	class FuncTest extends \PHPUnit_Framework_TestCase
	{
		...
		public function testThrottle()
		{
			...
			$wrapper();
			$wrapper();
			usleep(120 * 1000); $wrapper();
			usleep(140 * 1000); $wrapper();
			usleep(220 * 1000); $wrapper();
			usleep(240 * 1000); $wrapper();
		...
		{
			// -- Check whether all arguments get passed in the right order.
			$noop    = function() {};
			$wrapped = Func::wrap($noop, function($fn) { return func_get_args(); });
			$ret     = $wrapped(['whats', 'your'], 'vector', 'victor');

			$this->assertEquals([$noop, ['whats', 'your'], 'vector', 'victor'], $ret);

			// -- Check whether callables get passed properly.
			$greet     = function($name) {return 'hi: ' . $name;};
			$backwards = Func::wrap($greet, function(callable $func, $name) {return $func($name) . ' ' . strrev($name);});

			$this->assertEquals('hi: moe eom', $backwards('moe'));

			// -- Scope referencing.
			$inner     = function() {return 'Hello ';};
			$arr       = ['name' => 'Moe'];
			$arr['hi'] = Func::wrap($inner, function($fn) use ($arr) {return $fn() . $arr['name'];});

			$this->assertEquals('Hello Moe', $arr['hi']());

			// -- Completely wrap any variable arguments.
			$hello = function($name) { return 'hello: ' . $name; };
			$hi    = Func::wrap($hello, function($func) {
