<?php namespace nyx\utils;
	...
	class Str
	{
		 ...
		 * @param   bool            $strict     Set this to false to use allow for case-insensitive comparisons.
		 * @return  bool
		 */

		public static function contains($haystack, $needle, $encoding = null, $all = false, $strict = false)
		{
		...
		public static function encoding($str = null)
		{
			...
			if(null !== $str and false !== $encoding = mb_detect_encoding($str)) return $encoding;
		...
		{
			$encoding = $encoding ?: static::encoding($haystack);
			$haylen   = mb_strlen($haystack, $encoding);

			foreach((array) $needles as $needle)
			{
				if($needle == mb_substr($haystack, $haylen - mb_strlen($needle, $encoding), $encoding)) return true;
			}
		...
		public static function insert($str, $substring, $index, $encoding = null)
		{
			...
			if($index > mb_strlen($str, $encoding)) return $str;
		...
		public static function lcfirst($str, $encoding = null)
		{
			...
			if(isset($str[0])) $str[0] = mb_strtolower($str[0], $encoding ?: static::encoding($str));
		...
		public static function limit($str, $limit = 100, $encoding = null, $end = '...')
		{
			...
			if(mb_strlen($str, $encoding) <= $limit) return $str;
		...
		public static function startsWith($haystack, $needles, $encoding = null)
		{
			...
			foreach((array) $needles as $needle)
			{
				if(mb_strpos($haystack, $needle, 0, $encoding) === 0) return true;
			}
		...
		{
			// Need to check for the existence of the first character to avoid notices.
			if(isset($str[0])) $str[0] = mb_strtoupper($str[0], $encoding ?: static::encoding($str));
		...
		public static function words($str, $words = 100, $encoding = null, $end = '...')
		{
			...
			if(mb_strlen($str, $encoding) === mb_strlen($matches[0], $encoding)) return $str;
