<?php namespace nyx\framework\diagnostics\debug\delegates;
	...
	/**
	 * Debug Page Displayer
	 *
	 * Based on the PrettyPageHandler of Whoops.
	 *
	...
	class DebugPageDisplayer extends Displayer
	{
		/**
		 * @var array[]
		 */
		...
		{
			// Normally this Delegate should not got registered at all when in a CLI context, but just in case we
			// are going to enforce this check internally to prevent all that HTML output in the console.
			if(php_sapi_name() === 'cli') return null;

			// Get the 'pretty-template.php' template file
			$resources = $this->getResourcesPath();

			$templateFile = "$resources/debug-page-template.php";

			// @todo: Make this more reliable,
			// possibly by adding methods to append CSS & JS to the page
			$cssFile = "$resources/debug-page-displayer.css";

			// Prepare the $v global variable that will pass relevant information to the template
			...
			$v = (object) [
				...
				'frames'       => $trace,
				'hasFrames'    => !!count($trace),
				'handler'      => $this,
				'handlers'     => $inspector->getHandler()->getApplicable(),
		...
		protected function getFrameDescriptor()
		{
			return new displayer\FrameDescriptor();
		}
