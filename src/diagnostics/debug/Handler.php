<?php namespace nyx\diagnostics\debug;
	 ...
	 * Abstract Handler
	 *
	 * Provides means for concrete Handlers to deal with Conditions and debug Events.
	 *
	...
	abstract class Handler implements \nyx\events\interfaces\EmitterAware
	{
		/**
		 ...
		 *                          Error/Exception or arrays containing 'matches' and 'onMatch' keys with callables
		 ...
		 *
		 * @param   Condition|callable  $condition  Either a Condition instance or a 'matches' callable accepting one
		 ...
		 *                                          'matches' callable given as first argument returns true. This
		...
		public function apply($condition, callable $onMatch = null)
		{
			if(!$callable = is_callable($condition) and !$condition instanceof Condition)
			{
			...
			else
			{
				...
				$this->conditions[] = ['matches' => $condition, 'onMatch' => $onMatch];
			}
		...
		public function setAllowQuit($bool = null)
		{
			$this->allowQuit = (bool) $bool;
		}
		...
		protected function runConditions(\Exception $exception)
		{
			$prevent = false;
			...
			foreach($this->conditions as $condition)
			{
				...
				// away.
				elseif(true === call_user_func($condition['matches'], $exception, $this))
				{
				...
				if(($response & definitions\Signals::QUIT) === definitions\Signals::QUIT and $this->allowQuit) exit;
				...
				if(($response & definitions\Signals::PREVENT) === definitions\Signals::PREVENT) $prevent = true;
		 ...
		 * @return  \Exception|bool             Either an Exception when event emission occurred or false if no Emitter
		...
		protected function emitDebugEvent($name, \Exception $exception)
		{
			...
			if(null === $this->emitter) return false;
