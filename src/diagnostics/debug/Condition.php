<?php namespace nyx\diagnostics\debug;
	...
	class Condition
	{
		...
		{
			if(null !== $matches) $this->setMatches($matches);
			if(null !== $onMatch) $this->setOnMatch($onMatch);
		}
		...
		public function matches(\Exception $exception, Handler $handler)
		{
			return $this->matches ? call_user_func($this->matches, $exception, $handler) : false;
		}
		...
		public function setMatches(callable $code)
		{
			$this->matches = $code;
		...
		public function setOnMatch(callable $code)
		{
			$this->onMatch = $code;
		...
		public function apply(Handler $handler)
		{
			$handler->apply($this);
