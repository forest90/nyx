<?php namespace nyx\diagnostics\debug\handlers;
	...
	class Error extends debug\Handler implements interfaces\handlers\Error, interfaces\handlers\FatalError
	{
		...
		public function handle($type, $message, $file, $line, array $context)
		{
			...
			if(0 === $this->threshold) return false;
			...
			{
				// Fix for PHP bug #65322 {@see https://bugs.php.net/bug.php?id=65322}.
				if(!class_exists('nyx\diagnostics\debug\exceptions\Error'))
				{
					require dirname(__DIR__) . '/exceptions/Error.php';
				}
				...
				if($this->runConditions($exception)) return true;
		...
		public function onShutdown()
		{
			...
			// actually occurred.
			if(null === $error = error_get_last()) return;
			...
			if(0 === $this->threshold and !$this->isFatal($error['type'])) return;
			...
			if(is_array($handler) and $handler[0] instanceof interfaces\handlers\Exception)
			{
				// Fix for PHP bug #65322 {@see https://bugs.php.net/bug.php?id=65322}.
				if(!class_exists('nyx\diagnostics\debug\exceptions\FatalError'))
				{
					require dirname(__DIR__) . '/exceptions/FatalError.php';
				}
