<?php namespace nyx\diagnostics\debug\handlers;
	 ...
	 *
	 * Delegates (both callables and actual Delegates) can be named and referred to by the set name for white-and-black-
	 * listing and removal. Check {@see self::add()} and {@see self::set()} for information on the naming behaviour.
	 *
	...
	class Exception extends debug\Handler implements interfaces\handlers\Exception
	{
		...
		/**
		 * @var array   The delegates applicable for the next call to self::handle().
		 */
		...
		public static function register(interfaces\handlers\Exception $handler = null)
		{
		...
		public function handle(\Exception $exception)
		{
			...
			// self::emitDebugEvent() will return false when no Emitter is present. Otherwise we'll get the Exception
			// after it's been processed by Event Listeners so we need to overwrite it here.
			if(false !== $response = $this->emitDebugEvent(definitions\Events::DEBUG_EXCEPTION_BEFORE, $exception))
			{
				$exception = $response;
			}

			// First of all run all Conditions. The method will return true if we are to prevent further execution.
			if($this->runConditions($exception)) return;
			...
			if(!empty($delegates))
			{
				...
				// First of all inspect the Exception we got, which will give us an Inspector instance we can pass along.
				...
				foreach($delegates as $v)
				{
					...
					// after the loop.
					if(($response & definitions\Signals::QUIT) === definitions\Signals::QUIT) $quit = true;

					// QUIT includes STOP so this will catch both situations.
					if(($response & definitions\Signals::STOP) === definitions\Signals::STOP) break;
				}
			}
		...
		public function set($name, $delegate, $priority = 0)
		{
		 ...
		 *                                                      Also {@see self::getHighestPriority()}.
		 * @return  $this
		 * @throws  \DomainException                            When the given name (or when not given, the class name)
		 *                                                      is already set.
		 */

		public function add($delegate, $name = null, $priority = 0)
		{
			...
			{
				throw new \DomainException("A Delegate with the given name [$name] is already set.");
			}
		...
		public function remove($delegate, $all = true)
		{
		...
		public function whitelist($name)
		{
		...
		public function getApplicable()
		{
			...
			if(null !== $this->applicable) return $this->applicable;
			...
			if(!empty($this->whitelist)) $whitelist = $this->compileList($this->whitelist, $delegates);
			if(!empty($this->blacklist)) $blacklist = $this->compileList($this->blacklist, $delegates);

			// No need to proceed further if we didn't actually black/whitelist any currently added Delegates.
			if(empty($whitelist) and empty($blacklist)) return $this->applicable = $delegates;

			// Further checks.
			if(empty($whitelist)) return $this->applicable = array_diff($delegates, $blacklist);
			if(empty($blacklist)) return $this->applicable = $whitelist;
		...
		protected function sort(array &$delegates)
		{
		...
		protected function pushToList(&$list, $name)
		{
			// Handle arrays (recursively).
			if(is_array($name))
			{
				foreach($name as $single) $this->pushToList($list, $single); return $this;
			}
		...
		protected function compileList(&$list, $delegates)
		{
			$return = [];
