<?php namespace nyx\diagnostics\debug;
	...
	class Frame implements \Serializable, \JsonSerializable, core\interfaces\Arrayable, core\interfaces\Jsonable
	{
		/**
		 * The traits of a Frame instance.
		 */

		use core\traits\Jetget;
		...
		/**
		 * An array of cached file paths => file contents. Kept static to reduce IO overhead when multiple Frames are
		 * assigned to the same files.
		 *
		 * @var array
		 */
		...
		public function getFileContents()
		{
			...
			if(isset(static::$files[$this->file])) return static::$files[$this->file];
		 ...
		 * @return  array|null      The resulting array of lines or null when no file contents are available.
		 */

		public function getFileLines($offset = 0, $length = null)
		{
			// Return null right away if we are not able to grab the file contents for any reason.
			if(!$contents = $this->getFileContents()) return null;
			...
			return array_slice(explode("\n", $contents), (int) $offset, $length, true);
		}
		...
		/**
		 * Serializes this Frame. Ensures the args/context data is flat enough to be serialized and remain readable
		 * after unserializing.
		 *
		 * Allows for the implementation of \Serializable.
		 *
		 * @return  string
		 */
		...
		public function serialize()
		{
			...
			isset($data['args']) and $data['args'] = $this->flattenArgs($data['args']);

			return serialize($data);
		}

		/**
		 * Unserializes the given data and sets the current properties of this Frame to the unserialized values.
		 *
		 * Allows for the implementation of \Serializable.
		 *
		 * @param   string  $data   The serialized data to unserialize.
		 */
